...
(3) 提取所需内容并保存
----提取只包含评论的答案内容，目前发现有个字段comment_count用于
统计评论数，那么只需要提取评论数大于0的数据即可
...
def main():
    ...
    # 遍历json接口获取数据
    for i in range(len(json_url_list)):
        get_data(json_url_list[i])
